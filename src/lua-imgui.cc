...
ioSetMouseWheel(lua_State *L){
	...
	bool horizaton = lua_toboolean(L,2);
	ImGuiIO& io = ImGui::GetIO();
	if(horizaton){
		io.MouseWheelH += v;
	}
...
static int 
lshowDemo(lua_State *L){
	ImGui::ShowDemoWindow();
	return 0;
}

static int
lAddInputCharsUtf8(lua_State *L){
...
extern "C" int
luaopen_oolong_imgui(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "Create",lcreate},
		{ "NewFrame",lnewframe},
		...
		{ "AddInputCharsUtf8", lAddInputCharsUtf8},
		{ "ShowDemo",lshowDemo},
		{ NULL, NULL },
	};
